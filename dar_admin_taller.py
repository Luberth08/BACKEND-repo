...
async def set_admin(email: str):
    async with AsyncSessionLocal() as db:
        ...
        # Buscar rol admin
        res = await db.execute(select(Rol).where(Rol.nombre == ROL_ADMIN_TALLER))
        rol_admin = res.scalar_one_or_none()
        if not rol_admin:
            print(f"Error: El rol '{ROL_ADMIN_TALLER}' no existe en la base de datos.")
            return
        ...
        db.add(nuevo_rol)
        await db.commit()
        print(f"¡Éxito! Se le ha dado el rol de Administrador de Taller a {email}")

if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Uso: python dar_admin_taller.py <correo_del_usuario>")
    else:
